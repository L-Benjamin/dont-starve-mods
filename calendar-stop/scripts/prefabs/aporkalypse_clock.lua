local assets=
{
	...
	Asset("ANIM", "anim/pressure_plate_stop_build.zip"),
...
local plate_prefabs = 
{
	...
	["aporkalypse_stop_plate"] = {x = 8, z = 0},
}
...
local function make_master_fn()
	...
	inst.StopRewind = function ( )
		...
		local aporkalypse = GetAporkalypse()
		if aporkalypse and not aporkalypse.stop then
			inst.SoundEmitter:PlaySound("dontstarve_DLC003/common/objects/aporkalypse_clock/base_LP", "base_sound")
		end
		...
		inst:ListenForEvent("clocktick", function(world, data)
			...
			if aporkalypse then
				if inst.rewind then
					...
					if aporkalypse.stop then
						aporkalypse.stop_time = math.max(aporkalypse:GetBeginDate() - GetClock():GetTotalTime(), 0,001)
					end
				elseif aporkalypse.stop then
					if aporkalypse:IsActive() then
						aporkalypse:EndAporkalypse()
					end
	
					aporkalypse:ScheduleAporkalypse(GetClock():GetTotalTime() + aporkalypse.stop_time)
				end
			end
...
local function make_stop_plate()
	local inst = make_common_plate()
	inst.AnimState:SetBuild("pressure_plate_stop_build")

	inst.trigger = function()
		local aporkalypse = GetAporkalypse()

		if aporkalypse then
			aporkalypse.stop = not aporkalypse.stop
			if aporkalypse.stop then
				aporkalypse.stop_time = math.max(aporkalypse:GetBeginDate() - GetClock():GetTotalTime(), 0,001)
				inst.SoundEmitter:KillSound("base_sound")
			else
...
return MakeClock( 1, 1, 1),
	   ...
	   Prefab( "common/objects/aporkalypse_stop_plate", make_stop_plate, assets),
